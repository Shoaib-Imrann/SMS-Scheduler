...
export const sendScheduledCampaigns = async () => {
  ...
  const IST_OFFSET_MS = 5.5 * 60 * 60 * 1000;
  const nowIst = new Date(nowUtc.getTime() + IST_OFFSET_MS);
  const windowEnd = new Date(nowIst.getTime() + 5 * 60 * 1000);

  const campaigns = await campaignModel.find({
    status: 'scheduled',
    scheduleTime: { $gte: nowIst, $lt: windowEnd },
  });

  for (const c of campaigns) {
    const delay = new Date(c.scheduleTime).getTime() - Date.now();
    ...
    setTimeout(async () => {
      try {
        ...
        console.error("💥 Campaign execution failed:", err.message);
      }
    }, Math.max(0, delay)); // Just in case delay is negative
  }
};
